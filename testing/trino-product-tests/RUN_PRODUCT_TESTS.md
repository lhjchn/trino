...
```bash
mvnd -pl testing/trino-testing-services -DskipTests -Dair.check.skip-all=true install
```
...
```bash
mvnd -DskipTests -Dair.check.skip-all=true install
...
- Stale Trino image: tests run an older server image than your code changes.
  Fix: run `mvnd -DskipTests -Dair.check.skip-all=true install` and `(cd core/docker && ./build.sh)`.
