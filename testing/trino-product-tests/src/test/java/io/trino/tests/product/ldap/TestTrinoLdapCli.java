...
class TestTrinoLdapCli
{
            ...
            throws Exception
    {
        ...
        assertThat(result.getExitCode()).isNotZero();
        assertThat(result.getStderr()).contains("Invalid credentials");
    }
            ...
            throws Exception
    {
        ...
        assertThat(result.getExitCode()).isNotZero();
        assertThat(result.getStderr()).contains("Access Denied");
    }
    ...
    void shouldFailQueryForLdapWithoutPassword(LdapBasicEnvironment env)
            throws Exception
    {
        ExecResult result = env.executeCliWithoutPassword(
                env.getDefaultUser(),
                "--execute", SELECT_FROM_NATION);

        assertThat(result.getExitCode()).isNotZero();
        assertThat(result.getStderr()).contains("Authentication failed: Unauthorized");
    }
