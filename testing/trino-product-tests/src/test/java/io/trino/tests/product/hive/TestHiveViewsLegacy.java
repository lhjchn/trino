...
class TestHiveViewsLegacy
{
    ...
    void testSelectFromHiveViewWithoutDefaultCatalogAndSchema(HiveBasicEnvironment env)
    {
        ...
        // Query with explicit catalog and schema should work
        executeInSession(env, session -> {
            QueryResult result = session.executeQuery("SELECT count(*) FROM hive.default.no_catalog_schema_view");
            assertThat(result).containsOnly(row(1L));
        });
    }
    ...
    void testTimestampHiveView(HiveBasicEnvironment env)
    {
        ...
        executeInSession(env, session -> {
            // timestamp_precision not set - defaults to MILLISECONDS
    ...
    void testRunAsInvoker(HiveBasicEnvironment env)
    {
        ...
        env.executeHiveUpdate("CREATE VIEW run_as_invoker_view AS SELECT * FROM run_as_invoker");

        // Query with definer (default) should work
        executeInSession(env, session -> {
            QueryResult result = session.executeQuery("SELECT * FROM hive.default.run_as_invoker_view");
