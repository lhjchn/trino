...
{
    private static final String KAFKA_TABLE_DIR = "/etc/trino/kafka-tables";
    ...
    public void start()
    {
        ...
        // Build Trino container with Kafka catalog
        trino = TrinoProductTestContainer.builder()
                .withNetwork(kafka.getNetwork())
                .withNetworkAlias("trino")
                .withCatalog("kafka", Map.of(
                        "connector.name", "kafka",
                        "kafka.nodes", "kafka:9093",
                        "kafka.table-names", buildTableNames(),
                        "kafka.table-description-dir", KAFKA_TABLE_DIR))
                .build();

        // Copy table definition files to container
