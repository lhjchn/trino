...
class TestLdapTrinoJdbc
{
    ...
    {
        expectQueryToFail(env, env.getUserInEurope(), env.getLdapPassword(),
    ...
    {
        expectQueryToFail(env, env.getDefaultUser(), "wrong_password",
                "Authentication failed: Access Denied: Invalid credentials");
    }
    ...
    {
        expectQueryToFail(env, user, env.getLdapPassword(),
                format("Authentication failed: Access Denied: User [%s] not a member of an authorized group", user));
    }
